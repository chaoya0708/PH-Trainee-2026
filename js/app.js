  ...
  try {
    ...
    if (user && user.role === 'admin') {
      ...
      msg.innerHTML = `
        ...
        <h3 style="margin-bottom:10px;margin-top:0;">⚠️ 系統連線失效</h3>
        <p style="font-size:14px;font-weight:400;margin:0 0 20px 0;line-height:1.5;">
          資料庫拒絕連線：<br>
          <b style="color:#7f1d1d; display:block; margin-top:8px;">${err.message || err.toString()}</b>
          <br><br>
          <span style="font-size:12px; color:#555;">(這代表您剛才並未成功登入 Firebase 帳號，觸發了備用登入。請確認您的 Firebase 帳號已正確建立，並對網頁按下 Ctrl+F5 強制重新整理來清除舊快取。)</span>
        </p>
...
function renderDashboard() {
  ...
  const calCells = days.map((day, index) => {
    ...
    if (dept) {
      ...
      cellContent += `<span class="cal-dept-tag" style="border-color: ${borderColor}; color: var(--text-secondary); background: transparent;">${state.activeLanguage === 'zh' ? dept.nameZh : dept.name}</span>`;
    }
...
function renderAnalytics() {
  ...
  const leaderboardRows = trainees.map(tr => {
    ...
    const excludedDepts = tr.excludedDepartments || [];
    const trDepts = depts.filter(d => !excludedDepts.includes(d.id));
        ...
        <table class="analytics-table">
          <thead>
            <tr>
              ...
              ${user.role !== 'guest' ? '<th>Pulse Check</th>' : ''}
    ...
    const deptCards = Object.values(CONFIG.DEPARTMENTS).filter(d => !d.isRecordOnly && !excludedDepts.includes(d.id)).map(dept => {
      ...
      if (assessment) {
        if (user.role === 'trainee' && !assessment.visibleToTrainee) {
          ...
          `;
        } else if (user.role === 'guest' && dept.id !== user.departmentId) {
          assessmentHtml = `
            <div class="assessment-card" style="margin-top:14px;padding:8px 12px;background:rgba(16, 185, 129, 0.04);border:none;border-radius:10px;text-align:center;font-size:11px;color:var(--text-primary); font-weight: 600;">
              <i class="fi fi-rr-check-circle" style="color:var(--success); margin-right:4px;"></i> ${state.activeLanguage === 'zh' ? '考核已完成 (內容僅該單位可見)' : 'Assessment Completed (Private to Dept)'}
            </div>
          `;
        } else {
...
window.openEditObservation = function (id) {
  ...
  // Handle translation field (admin only)
  const currentUser = Auth.getCurrentUser();
  if (currentUser && currentUser.role === 'admin') {
    $('editObsTranslationGroup').style.display = 'block';
    if (!window.editObsTranslationQuill) {
      window.editObsTranslationQuill = new Quill('#editObsTranslationEditor', {
        theme: 'snow',
        modules: {
          toolbar: [
            [{ 'header': [1, 2, 3, false] }, { 'size': ['small', false, 'large', 'huge'] }],
    ...
    let transContent = obs.officialTranslation || '';
    if (!transContent.includes('<p>') && !transContent.includes('<br>')) {
      transContent = transContent.replace(/\n/g, '<br>');
    }
    window.editObsTranslationQuill.root.innerHTML = transContent;
  } else {
    $('editObsTranslationGroup').style.display = 'none';
  }
...
window.saveEditedObservation = async function () {
  ...
  const currentUser = Auth.getCurrentUser();
  if (currentUser && currentUser.role === 'admin' && window.editObsTranslationQuill) {
    data.officialTranslation = window.editObsTranslationQuill.root.innerHTML;
  }
