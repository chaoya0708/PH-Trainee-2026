...
function renderDashboard() {
  ...
  const dayNames = {
    ...
    tg: ['Lun','Mar','Miy','Huw','Biy','Sab','Lin']
  ...
  const calCells   = days.map((day, index) => {
    ...
    const dateStr = state.activeLanguage === 'zh' ? `${monthName}${dateNum}日` : `${dateNum} ${monthName}`;
    
    return `
      <div class="cal-cell ${active} ${isWeekend}" onclick="window.selectCalDate('${day}')">
        <span class="cal-date">${dateStr}</span>
        ${dept ? `<span class="cal-dept-tag" style="background:${dept.color}">${state.activeLanguage === 'zh' ? dept.nameZh : dept.name}</span>` : ''}
      </div>
    `;
  }).join('');
  ...
  container.innerHTML = `
    ...
    <div class="glass-card" style="width:100%;margin-bottom:20px;">
      <div class="card-header" style="flex-wrap:wrap;gap:12px;">
        <h3>${t('weeklySchedule')} — ${viewUser.name}</h3>
        <div style="display:flex;align-items:center;gap:8px;">
          <label style="font-size:12px;font-weight:700;color:var(--text-muted);">${state.activeLanguage === 'zh' ? '選擇日期: ' : 'Select Date: '}</label>
          <input type="date" class="form-control" style="width:140px;padding:4px 8px;font-size:13px;" 
            id="schedDatePicker" value="${state.selectedDate}" onchange="window.selectCalDate(this.value)">
        </div>
      </div>
    ...
    <div class="glass-card" style="width:100%;">
      <div class="card-header"><h3>${viewUser.name}</h3></div>
      <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px;">
        <div class="user-avatar" style="width:52px;height:52px;font-size:26px;">${viewUser.avatar || '👤'}</div>
        <div>
          <div style="font-weight:700;font-size:16px;">${viewUser.name}</div>
          <span class="user-role-badge">${t('loginTrainee')}</span>
        </div>
      </div>
      <div class="progress-row" style="margin-bottom:18px;">
        <div class="progress-label"><span>${t('overallProgress')}</span><span>${overallPct}%</span></div>
        <div class="progress-bar"><div class="progress-fill" style="width:${overallPct}%;"></div></div>
      </div>
      ${viewUser.bio ? `<p style="font-size:11px;color:var(--text-secondary);line-height:1.5;">${viewUser.bio}</p>` : ''}
    </div>
  `;
}

window.selectViewTrainee = function(id) {
...
window.saveSchedule = async function(traineeId, date) {
  const deptEl = document.getElementById('editDept');
  const objEl  = document.getElementById('editObjective');
  if (!deptEl || !objEl) return;
  const dept      = deptEl.value;
  const objective = objEl.value.trim();
  if (!objective) { showToast('請輸入學習目標 / Please enter a learning objective.', 'error'); return; }

  showLoading();
  try {
    await Api.updateSchedule(traineeId, date, dept, objective);
    state.schedules     = await Api.getAllSchedules();
    state.editingSchedule = false;
    showToast('排程已更新！ Schedule updated!', 'success');
    renderDashboard();
  } catch(err) {
    showToast('儲存失敗 / Save failed: ' + err.message, 'error');
  } finally {
    hideLoading();
  }
};
